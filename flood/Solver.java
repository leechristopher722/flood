...
class Solver {
    ...
    static int[] chooser(Model model, int depth) {
        ...
        for (bestMove = 0; bestMove < model.ncolors(); bestMove++) {
            model.setActiveRegionColor(bestMove);
            if (model.activeRegionSize() == winningScore) {
                return new int[] {bestMove, model.activeRegionSize()};
            } else if (maxSize < model.activeRegionSize()) {
