...
class Model {
    ...
    boolean[] colorsPresent() {
        boolean[] result = new boolean[ncolors()];
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        ...
        for (Place neighbor : neighbors) {
            if (get(neighbor) == color && !result.contains(neighbor)) {
                result.add(neighbor);
                findRegion(neighbor, color, result);
            }
        }
        return result;
    }
    ...
    void setActiveRegionColor(int newColor) {
        ...
        for (Place activeCells : _active) {
            _cells[activeCells.row][activeCells.col] = newColor;
            newActive = findRegion(activeCells, newColor, newActive);
        }
        if (!solved()) {
            _current += 1;
        }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> neighbors = new HashSet<>();
        for (Place actCell : _active) {
            forNeighbors(actCell, (p) -> {
                if (get(p) == color && actCell != p && !_active.contains(p)) {
                    neighbors.add(p);
                }
            });
        }
